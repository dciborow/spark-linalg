...
class IndexedRowMatrix(val rows: Dataset[IndexedVector],
  ...
  def multiply(other: IndexedColMatrix): CoordinateMatrix = {
    ...
    //    val matTest = rows.joinWith(other.rows,
    //      rows.col("index") <=> other.rows.col("index") || rows.col("index") =!= other.rows.col("index"), "cross")
    //      .mapPartitions((tuples) => {
    //        val mat1 = tuples.map((vectors) => {
    //          vectors._1.vector
    //        })
    //        val mat2 = tuples.map((vectors) => {
    //          vectors._2.vector
    //        })
    //        val mat1Dense: DenseMatrix  = DenseMatrix(mat1)
    //        val mat2Dense: DenseMatrix = DenseMatrix(mat2)
    //        mat1Dense.multiply(mat2Dense).asBreeze.iterator
    //        tuples
    //      })
    ...
    //    new CoordinateMatrix(
    //      rows.joinWith(other.rows,
    //        rows.col("index") <=> other.rows.col("index") || rows.col("index") =!= other.rows.col("index"), "cross")
    //        .mapPartitions(_.map((vectors) => {
    //          val iVec: Vector = vectors._1.vector
    //          val jVec = vectors._2.vector
    //          val product = (iVec, jVec) match {
    //            case (DenseVector(vs), DenseVector(vs2)) => {
    //              val ibdv = new BDV[Double](iVec.toArray)
    //              val jbdv = new BDV[Double](jVec.toArray)
    ...
    //            }
    //          }
    //          MatrixEntry(vectors._1.index, vectors._2.index, product)
    //        }))
    //    )
  }
